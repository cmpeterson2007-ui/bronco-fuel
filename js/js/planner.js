...
// Buster's Kitchen served stations hand over one serving at a time. The
// self-serve stations can be revisited for additional servings.
const SINGLE_SERVING_STATIONS = /\b(homestyle|flame|la\s*mesa|create|delicious\s+without)\b/i;

export function maxServingsForItem(item) {
  return SINGLE_SERVING_STATIONS.test(String(item?.station || '')) ? 1 : MAX_SERVINGS;
}
...
function buildLaMesaPlate(pool, budget, usageMap, options = {}) {
  ...
  for (let guard = 0; guard < 4; guard++) {
    ...
    for (const item of mesa) {
      ...
      if (count >= maxServingsForItem(item)) continue;
...
function buildFlameOmelet(pool, budget, usageMap, options = {}) {
  ...
  const base = [{ item: egg, servings: 1 }];
  let total = { calories: egg.calories, protein: egg.protein, carbs: egg.carbs, fat: egg.fat };
...
export function optimizeMeal(pool, budget, usageMap = new Map(), options = {}) {
  ...
  for (;;) {
    ...
    for (let i = 0; i < pool.length; i++) {
      if (servings[i] >= maxServingsForItem(pool[i])) continue;
  ...
  while (improved && guard++ < 60) {
    ...
    for (let i = 0; i < pool.length; i++) {
      ...
      for (let j = 0; j < pool.length; j++) {
        if (j === i && servings[j] + 1 > maxServingsForItem(pool[j])) continue;
        if (servings[j] >= maxServingsForItem(pool[j])) continue;
